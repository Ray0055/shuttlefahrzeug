...
protected:
    ...
    double max_detected_distance_ = 50.0;
    ...
    void detectLanes_poly()
    {
        ...
        cv::Mat BEV_img_color = birdEyeView_fullscreen( region_of_interest, true );

        cv::Mat color_filtered_img = colorDetection( region_of_interest );

        cv::Mat edge_detected_img = edgeDetection( color_filtered_img );

        cv::Mat BEV_image_ = birdEyeView_fullscreen( color_filtered_img, true );

        imshow( "BEV_image", BEV_image_ );

        if( right_lane.detected && left_lane.detected )
        {
            line_detection_by_previous( BEV_image_, BEV_img_color, 15, left_lane, right_lane );
        }
        else
        {
            line_dection_by_sliding_window( BEV_image_, BEV_img_color, 15, left_lane, right_lane );
        }

        draw_detected_lane_onto_road( original_image_, left_lane, right_lane );
        saveToCSV_PID( right_lane.last_fit_pixel.at<double>( 0, 0 ),
                       right_lane.last_fit_pixel.at<double>( 1, 0 ),
                       right_lane.last_fit_pixel.at<double>( 2, 0 ),
                       left_lane.last_fit_pixel.at<double>( 0, 0 ),
                           left_lane.last_fit_pixel.at<double>( 1, 0 ),
                       left_lane.last_fit_pixel.at<double>( 2, 0 ) );
...
void laneDetectionRobust( vector<Point> lane, string lane_name )
{
    ...
    else
    {
        ...
        double slope_sum =
            std::accumulate( lane_slope_history_ptr->begin(), lane_slope_history_ptr->end(), 0.0 );
        double slope_mean = slope_sum / lane_slope_history_ptr->size();

        double bias_sum =
            std::accumulate( lane_bias_history_ptr->begin(), lane_bias_history_ptr->end(), 0.0 );
...
void steeringControl( Mat img )
{
    ...
    /*saveToCSV_PID( steer_error_P, steer_error_I_, steer_error_D,
                   steer_output / ( img.cols / 2 ) );*/
...
{
    double distance_target = 0.6 * ego_velocity_;
...
void saveToCSV_PID( double right1, double right2, double right3, double left1,
                    double left2, double left3)
{
    std::ofstream lines_data;
    lines_data.open( "D:\\200_Projekte\\distanceData.csv", ios::app );

    lines_data << right1 << "," << right2 << "," << right3 << "," << left1 << "," << left2 << ","
               << left3<< "," << endl;
